#!/usr/bin/env python3
"""
catalog_builder_team_repo.py — Mentora team-repo catalog builder & validator

Walks team repos (locally or via the GitHub API), reads each skills.md,
validates against the v1 contract, and writes a build report that groups
all errors and warnings by team — so each team knows exactly what they
need to fix.

Because team repos are the authoritative source of skill ownership, the
report here uses per-team tables (one table per repo/team) in the Issues
section. This makes it easy to share a section with a specific team.
...
Usage:
  python catalog_builder_team_repo.py
  python catalog_builder_team_repo.py --local /path/to/parent/dir
  python catalog_builder_team_repo.py --local . --report build_report.md

For building the runtime catalog from the mentora_skills package use:
  catalog_builder_mentora_skills.py
"""
...
# ---------------------------------------------------------------------------
# Build report — per-team layout
# ---------------------------------------------------------------------------

def render_report(result: BuildResult) -> str:
    lines = ["# Catalog build report (team repos)", ""]
    ...
    if non_ready:
        ...
        for s in sorted(non_ready, key=lambda x: (x.get("owner_team", ""), x.get("source_path", ""))):
            team = s.get("owner_team") or s.get("source_repo", "?")
            path = s.get("source_path", "?")
            status = s.get("status", "?")
            reason = s.get("status_reason") or ""
            lines.append(f"| `{team}` | `{path}` | {status} | {reason} |")
    else:
        lines.append("_All skills are ready — no stubs or broken skills._")
    lines.append("")

    # Issues — one table per team/repo
    lines += ["## Issues by team", ""]

    # Group by owner_team (falls back to source_repo)
    issues_by_team: dict = {}
    for path, issues in sorted(result.issues_by_skill.items()):
        if not issues:
            continue
        repo_name = path.split("/")[0]
        # Look up the owner_team for this repo from the catalog
        team_name = next(
            (s.get("owner_team") or repo_name
             for s in result.catalog if s.get("source_repo") == repo_name),
            repo_name,
        )
        issues_by_team.setdefault(team_name, {}).setdefault(repo_name, []).append((path, issues))

    if not issues_by_team:
        lines.append("_All skills passed validation cleanly._")
    else:
        for team_name in sorted(issues_by_team):
            lines.append(f"### {team_name}")
            lines.append("")
            lines.append("| Repo | Skill | Severity | Field | Message |")
            lines.append("|------|-------|----------|-------|---------|")
            for repo_name in sorted(issues_by_team[team_name]):
                for path, issues in issues_by_team[team_name][repo_name]:
                    for i in issues:
                        severity = "**ERROR**" if i.severity == "error" else "**warn**"
                        lines.append(
                            f"| `{repo_name}` | `{path}` | {severity} "
                            f"| `{i.field}` | {i.message} |"
                        )
            lines.append("")
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Validate team-repo skills and produce a per-team build report."
    )
    ...
    if result.duplicate_rejections:
        ...
        for rej in result.duplicate_rejections:
            print(
                f"  skill_id='{rej['skill_id']}': '{rej['rejected_repo']}' rejected "
    ...
    if args.strict:
        ...
        if broken:
            ...
            for s in broken:
                print(
                    f"  {s.get('owner_team', '?')} / {s.get('source_path', '?')}: "
